...
add_global_pre_hook "require_credential_rotation" do
  ...
  # "chef-server-ctl" is a wapper that runs "opscode /opt/opscode/embedded/service/omnibus-ctl $command"
  # so we'll look for that in ARGV
  return if ARGV == %w{opscode /opt/opscode/embedded/service/omnibus-ctl rotate-shared-secrets}
